...
The two models are not significantly differ. So choose the parsimonious model 4:
...
```{r, echo=FALSE}
# failed to converge
m_d_level1_slope3_1 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + Fairness_score + (Fairness_score|ID) + (ReappraisalDirection|ID), data = allData, family = binomial, control = glmerControl(calc.derivs = FALSE, optCtrl = list(maxfun = 2e4)), na.action = "na.fail")
...
print(m_d_level1_slope3_1)

m_d_level1_slope3_1 <- all_fit(m_d_level1_slope3_1)
m_d_level1_slope3_2 <- all_fit(m_d_level1_slope3_2)
```
The best model (only converged) with two random $effects Fairness_score|ID) + (ReappraisalDirection|ID)$ is 

$$logit(accept) = Reappraisal\,Direction + scale(total\, stake) + Reappraisal\,Direction|ID +\\ Fairness\, score |ID$$

```{r, echo=FALSE}
summary(m_d_level1_slope3_2$bobyqa.)
```

## Compare models with only level 1 predictors (different random effects)
This model is better than others.
$$logit(accept) = Reappraisal\,Direction + scale(total\, stake) + Reappraisal\,Direction|ID +\\ Fairness\, score |ID$$
...
m_level12_interceptOnly.mod1 <- update(m_level12_interceptOnly, .~. - MARITALTEXT - scale(PROTECT2AGE))

summary(m_level12_interceptOnly)
```

# Random intercept & random slope (Level 1 + Level 2)
## Simple Model(with only group predictor)
**Model**
$$logit(accept) = Reappraisal\,Direction + total\, Stake + Fairness\,score + group4$$
Comparing the simple models:
```{r, echo=FALSE}
# group 4
...
m_level12_12.mod1 <- update(m_level12_12, .~. - scale(totalStake))
m_level12_12.mod1 <- all_fit(m_level12_12.mod1)
m_level12_12.mod2 <- update(m_level12_12, .~. - Fairness_score)
...
# compare the models
modellist6 <- model.sel(m_d_level1_slope3_2$bobyqa., m_level12_1, m_level12_11, m_level12_12, m_level12_2, m_level12_22, m_level12_23, m_level12_12.mod1, m_level12_12.mod2)

print(modellist6)

```

Group 4 or 5 does not make any difference to the acceptance. 

*Random effect*: The model with two random effect (Fairness_score|ID) + (ReappraisalDirection|ID) has significantly smaller AIC than the model with one random effect (ReappraisalDirection|ID) and the model with (Fairness_score|ID). 

So the best model in the simple model is 
$$accept ~ Reappraisal\,Direction + scale(total\,Stake) + Fairness\,score\\ + group4 + (Fairness\,score|ID)+ (Reappraisal\,Direction|ID)$$

```{r}
summary(m_level12_12)
```

Adding the predictor $group$ does influence the model:

```{r}
anova(m_d_level1_slope3_2$bobyqa., m_level12_12)
```
...
The global model with interation effect (income x race) is:
```{r}
...
The global model with interation effect (income x gender) is:
```{r}
...
The global model without interation effect is:
```{r}
...
d_model_level12.mod1 <- lapply(dredge(model_level12_noInter1, evaluate = FALSE), eval, subset["PROTECT2AGE", "MARITALTEXT"] == FALSE)

lapply(dredge(model_level12_noInter1, evaluate = FALSE, eval, subset = delta < 2))

## Best model
print(summary(get.models(d_model_level12,1)[[1]]))
print(get.models(d_model_level12, subset = delta < 2))
```

The summary of all alternatives by model with random effect (Reappraisal Direction|ID) + (Fairness score|ID) of no interaction model:

## average the best model of the non interaction models
$$allData$AcceptOffer ~ ReappraisalDirection + scale(totalStake) + Fairness_score + group4 + scale(PROTECT2AGE) + scale(HRSD_NO_SUI) + scale(HouseholdIncome) + RACETEXT + GENDERTEXT + scale(MMSE) + scale(EDUCATION) + MARITALTEXT + (ReappraisalDirection|ID)$$
```{r}
summary(model.avg(d_model_level12, subset = delta < 2))
```

All alternatives by model with random effect (Reappraisal Direction|ID) + (Fairness score|ID)
...
```{r}
# As a rule of thumb,  delta aic < 2 suggests substantial evidence for the model
...
# whereas a delta aic > 10 indicates that the model is very unlikely (Burnham and Anderson 2002)
