...
$$Accept\,Offer = Reappraisal\,Direction + scale(total\,Stake) + Fairness\,score + group + (1|ID)$$
```{r}
...
# try various optimizers
m3 <- all_fit(m3)
summary(m3)
```

Add subject level variables.
$$Accept\,Offer = Reappraisal\,Direction + scale(total\,Stake) + Fairness\,score + group + VAR + (1|ID)$$
```{r}
...
```
No subject level variables have significant influence on the gamble acceptance.
...
Screening all alternative models of the level 1 + group models, including interaction effect
```{r}
...
m2_best <- glmer(allData$AcceptOffer ~ ReappraisalDirection + Fairness_score + group4 + scale(totalStake) + ReappraisalDirection:scale(totalStake) + (1|ID), data = allData, family = binomial, control = glmerControl(optimizer = "bobyqa"))
...
All the alternative models are:
```{r}
m4_level1 <- glmulti(allData$AcceptOffer ~ ReappraisalDirection * scale(totalStake) * Fairness_score, level = 1, fitfunction = glmer.glmulti, random = "+ (block|ID)", method = "g", data = allData, confsetsize = 8)

m4_models <- weightable(m4_level1)
print(m4_models)
```

## Level 1 model with interaction effect 
```{r}
m5 <- glmulti(allData$AcceptOffer ~ ReappraisalDirection * scale(totalStake) * Fairness_score, level = 2, fitfunction = glmer.glmulti, random = "+ (block|ID)", method = "g", data = allData)

# look at the alternative models
m5_models <- weightable(m5)
print(m5_models)
```

# Level 1 model with $group$ predictor
## No interaction model
$$Accept\,Offer = Reappraisal\,Direction + scale(total\,Stake) + Fairness\,score + group + (block|ID)$$
```{r}
m6 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + scale(totalStake) + Fairness_score + group4 + (block|ID), family = binomial, data = allData)
...
Add subject level variables.
$$Accept\,Offer = Reappraisal\,Direction + scale(total\,Stake) + Fairness\,score + group + VAR + (block|ID)$$
```{r}
m_education1 <- update(m6, .~. + scale(EDUCATION))
m_income1 <- update(m6, .~. + scale(IncomePerCapita))
m_exit1 <- update(m6, .~. + scale(EXIT))
m_mmse1 <- update(m6, .~. + scale(MMSE))
m_nonplan1 <- update(m6, .~. + scale(BIS_NONPLAN))
m_urgency1 <- update(m6, .~. + scale(urgency))
m_interpersonal1 <- update(m6, .~. + scale(IIP15INTAMBV))
...
```{r}
m7 <- glmulti(allData$AcceptOffer ~ ReappraisalDirection * scale(totalStake) * Fairness_score * group4, level = 2, fitfunction = glmer.glmulti, random = "+ (block|ID)", method = "g", data = allData)

m7_1 <- glmulti(allData$AcceptOffer ~ ReappraisalDirection * scale(totalStake) * Fairness_score * group5, level = 2, fitfunction = glmer.glmulti, random = "+ (block|ID)", method = "g", data = allData)

print(m7)
```  

The best model of the interaction model is 
```{r}

```

Add subject level variable into the best model
```{r}
m7_best_education <- update(m7_best, .~. + scale(EDUCATION))
m7_best_mmse <- update(m7_best, .~. + scale(mmse))
m7_best_exit <- update(m7_best, .~. + scale(EXIT))
m7_best_nonplan <- update(m7_best, .~. + scale(nonplan))
m7_best_urgency <- update(m7_best, .~. + scale(urgency))
m7_best_income <- update(m7_best, .~. + scale(income))
m7_best_interpersonal <- update(m7_best, .~. scale(IIP15INTAMBV))
```
