...
df = allData[, c("HRSD_NO_SUI", "PROTECT2AGE", "HouseholdIncome", "totalStake", "DRS")]
...
```{r} 
# basic model + loop all alternatives 
## random intercept, fixed predictors at 1 and 2
model_basic <- glmer(allData$AcceptOffer ~ ReappraisalDirection + totalStake + Fairness_score + group4 + (1|ID), data = allData, family = binomial)
print(model_basic)
options(na.action = "na.fail")
dd <- dredge(global.model = model_basic, rank="AICc")
print(dd)

## Best model
print(summary(get.models(dd,1)[[1]]))

```

```{r}
# comparing the basic models
mb1 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + totalStake + (1|ID), data = allData, family = binomial)
mb2 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + Fairness_score + (1|ID), data = allData, family = binomial)
mb3 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + group4 + (1|ID), data = allData, family = binomial)
mb4 <- glmer(allData$AcceptOffer ~ totalStake + Fairness_score + (1|ID), data = allData, family = binomial)
mb5 <- glmer(allData$AcceptOffer ~ totalStake + group4 + (1|ID), data = allData, family = binomial)
mb6 <- glmer(allData$AcceptOffer ~ Fairness_score + group4 + (1|ID), data = allData, family = binomial)
print(summary(mb6))

mb7 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + totalStake + Fairness_score + (1|ID), data = allData, family = binomial)
mb8 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + Fairness_score + group4 + (1|ID), data = allData, family = binomial)
mb9 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + totalStake + group4 + (1|ID), data = allData, family = binomial)
mb10 <- glmer(allData$AcceptOffer ~ totalStake + Fairness_score + group4 + (1|ID), data = allData, family = binomial)

anova(m_framing, m_stakeSize, m_fair, m_group4, mb1, mb2, mb2, mb3, mb4, mb5, mb6, mb7, mb8, mb9, mb10, model_basic)
print(summary(mb2))
print(summary(mb8))
```

```{r}
# models includes the level 2 predictors + loop loop all alternatives 
## random intercept, fixed predictors at 1 and 2
m1 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + scale(totalStake) + Fairness_score + group4 + scale(PROTECT2AGE) + scale(HRSD_NO_SUI) + scale(HouseholdIncome) * RACETEXT + GENDERTEXT + (1|ID), data = allData, family = binomial)

options(na.action = "na.fail")
all_models <- dredge(global.model = m1, rank="AICc")
print(all_models)

## Best model
print(summary(get.models(all_models,1)[[1]]))
### OR
print(aicrank[1])
...
m3 <- glmer(allData$AcceptOffer ~ ReappraisalDirection + totalStake + Fairness_score + Gender + HouseholdIncome + PROTECT2AGE + group4 + (1 + ReappraisalDirection|ID), data = allData, family = binomial)
print(summary(m3))
...
```{r}
# longitudinal model
# use reappraisalDirection as the time series.
