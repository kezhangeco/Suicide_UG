...
export2csv(t1, file = "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/demo4.csv")
...
m1g <-glmer(
  allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + ReappraisalDirection * group4 + scale(fairLR) * group4 + scale(totalStake) * group4 +  (1 | ID/block),
  family = binomial,
  data = allData,
  control = glmerControl(optimizer = "bobyqa"))
...
mc1 <-
  ...
  allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + 
    ReappraisalDirection * group4 + scale(fairLR) * group4 + 
    scale(totalStake) * group4 +
    ReappraisalDirection*scale(EXIT)*group4 + 
    scale(EDUCATION)*group4*ReappraisalDirection + 
    scale(IIP15INTSEN)*group4*ReappraisalDirection +
    Gender*ReappraisalDirection*group4 +
    scale(IncomePerCapita) * group4*ReappraisalDirection +
    scale(BIS_NONPLAN) * group4 * ReappraisalDirection + 
    scale(HRSD_NO_SUI) * group4 * ReappraisalDirection+ 
    scale(IIP15AGRESS)*group4*ReappraisalDirection +
    scale(IIP15INTAMBV)*group4*ReappraisalDirection +
    (1 | ID/block),
  family = binomial,
  data = allData,
...
a <- lapply(dredge(glmer(
  ...
  family = binomial,
  data = allData,
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e4))), evaluate = FALSE, fixed = c("ReappraisalDirection * scale(fairLR) * scale(totalStake)", "ReappraisalDirection * group4", "scale(fairLR) * group4", "scale(totalStake) * group4")), eval)

mc2 <-
  glmer(
  allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + 
    ReappraisalDirection * group4 + scale(fairLR) * group4 + 
    scale(totalStake) * group4 + scale(EXIT) + 
    scale(EDUCATION)+ 
    scale(IIP15INTSEN)+
    Gender + scale(IncomePerCapita)+
    scale(BIS_NONPLAN) + 
    scale(HRSD_NO_SUI) + 
    scale(IIP15AGRESS) +
    scale(IIP15INTAMBV) +
...
m1gSSI_1 <-
  glmer(
  allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + ReappraisalDirection *
  group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + scale(`SSI.BL.CURRENT`) + (1 | ID/block),
  family = binomial,
  data = allData,
  control = glmerControl(optimizer = "bobyqa",optCtrl = list(maxfun = 2e4)))
stargazer(m1gSSI_1, type = "text", title = "AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + ReappraisalDirection * group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + scale(`SSI.BL.CURRENT`) + (1 | ID/block)", out = "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/SSIBLCURRENT.text")
