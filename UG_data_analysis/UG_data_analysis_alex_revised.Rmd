...
<<<<<<< HEAD
  ...
  group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + 
      ReappraisalDirection*scale(EXIT) + (1 |
  ID),
  family = binomial,
  data = allData,
  nAGQ = 0,  
...
pdf("UGcontext*Group_w_EXIT.pdf", width=8, height=6)
lsm <- lsmeans(m1gm, "ReappraisalDirection", by = "group4")
plot(lsm, horiz = F)
dev.off()
...
```{r, individual factors FIX LATER}
...
# group4
## models loop through all vars by "rating = ReappraisalDirection*group4 + i + (1|ID)"
...
mReact4list1 <- lapply(varlist1, function(x) {
  lmer(substitute(scale(Rating) ~ ReappraisalDirection*group4 + i + (1|ID), list(i = as.name(x))), data = emoRatingScale)
})
mReact4list1_summ <- lapply(mReact4list1, summary)
print(mReact4list1_summ)
...
mReact4list2 <- lapply(varlist2, function(x) {
  lmer(substitute(scale(Rating) ~ ReappraisalDirection*group4*i + (1|ID), list(i = as.name(x))), data = emoRatingScale)
})
mReact4list2_summ <- lapply(mReact4list2, summary)
print(mReact4list2_summ)

for (i in mReact4list2_summ) {
  print(i$call)
  stargazer(i$coefficients, type = "text", title = paste("Rating ~ ReappraisalDirection*group4*", i, "+(1|ID)"),
...
# group4 graphs
pltFun <- function(x){
  lsm <- lsmeans(x, "ReappraisalDirection", by = c("group4"))
...
plt_NonInter1 <- lapply(mReact4list1, pltFun)
print(plt_NonInter1)
for (i in plt_NonInter1) {
  for (j in varlist) {
    #jpeg(paste('/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/rating=ReappraisalDirection*group4+', j, '.jpg'))
    plt_NonInter <- i + ggtitle(paste("Rating=group4*ReappraisalDirection+", j))
    dev.off()
    print(plt_NonInter)
  }
}

plt2 <- lapply(mReact4list1, pltFun1)
print(plt2)

## interaction model graphs
plt_Inter1 <- lapply(mReact4list2, pltFun)
print(plt_Inter1)

for (i in plt_Inter1) {
  for (j in varlist) {
    plt_Inter <- i + ggtitle(paste("Rating=group4*ReappraisalDirection*", j))
    dev.off()
    print(plt_Inter)
  }
}

plt1 <- lapply(mReact4list2, pltFun1)
print(plt1)
