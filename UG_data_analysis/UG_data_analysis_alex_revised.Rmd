...
m0r <- glmer(allData$AcceptOffer ~ ReappraisalDirection + scale(totalStake) + fairLR + (1|ID), family = binomial, data = allData)
summary(m0r)
...
m1 <- glmer(allData$AcceptOffer ~ ReappraisalDirection*fairLR*scale(totalStake) + (1|ID), family = binomial, data = allData, control = glmerControl(optimizer = "bobyqa"))
summary(m1)
car::Anova(m1)
anova(m0r,m1)
...
m1ge <-
  ...
  group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + 
      ReappraisalDirection*EDUCATION + (1 |
  ID),
  family = binomial,
  data = allData,
  control = glmerControl(optimizer = "bobyqa"))
