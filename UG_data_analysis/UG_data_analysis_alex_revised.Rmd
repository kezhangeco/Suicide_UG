...
## add context*fair*Group -- too complex, would not bother including
# m1g1 <-glmer(
#   allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + scale(fairLR) * ReappraisalDirection *
#   group4 + scale(totalStake) * group4 +  (1 |
#   ID/block),
#   family = binomial,
#   data = allData,
#   
#   control = glmerControl(optimizer = "bobyqa"))
#   summary(m1g1)
# car::Anova(m1g1, type = 'III')
# anova(m1g,m1g1)
# pdf("UGfair*context*group.pdf", width=10, height=16)
# lsm <- lsmeans(m1g, "fairLR", by = c("group4","ReappraisalDirection"), at = list(fairLR = c(-2.66,-1.22,0.22)))
# plot(lsm)
# dev.off()
...
m1ge <-
  ...
  group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + 
      ReappraisalDirection*scale(EDUCATION) + (1 |
  ID),
  family = binomial,
  data = allData,
