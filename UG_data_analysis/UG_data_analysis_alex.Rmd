...
Select the best model from the no interaction and interaction models:
```{r}
modellist1 <- model.sel(m3, m2_4interaction, m2_3interaction1, m2_3interaction2, m2_2interaction1, m2_2interaction2)
print(modellist1)
```

### the (best) interaction model with the subject level variable
```{r}
m_education1 <- update(m3, .~. + scale(EDUCATION))
m_income1 <- update(m3, .~. + scale(IncomePerCapita))
m_exit1 <- update(m3, .~. + scale(EXIT))
m_mmse1 <- update(m3, .~. + scale(MMSE))
m_nonplan1 <- update(m3, .~. + scale(BIS_NONPLAN))
m_urgency1 <- update(m3, .~. + scale(urgency))
m_interpersonal1 <- update(m3, .~. + scale(IIP15INTAMBV))
