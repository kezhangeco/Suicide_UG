...
# separate groups
depression <- subset(demoQuestion, group4 == "depression")
control <- subset(demoQuestion, group4 == "control")
ideator <- subset(demoQuestion, group4 == "ideator")
attempter <- subset(demoQuestion, group4 == "attempter")
attHL <- subset(allData, group5 == "AttempterHL")
attLL <- subset(allData, group5 == "AttempterLL")
t.test(depression$IIP15AGRESS,control$IIP15AGRESS)
...
nrow(subset(task, task$Fairness_score == 1 | task$Fairness_score == 2))
nrow(subset(task, task$Fairness_score == 3 | task$Fairness_score == 4 | task$Fairness_score == 5))
nrow(subset(task, task$AcceptOffer == 0))
```
...
ggplot(lsmDf, aes(ReappraisalDirection, lsmean)) +
  ...
  geom_point() +
  xlab("Context") +
  ylab("Logit Probability of Offer Acceptance") +
...
tiff('/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/NoIndividual/UGcontext*UGfair.tiff', 
     units="in", height = 5, width = 9, res=300)
lsm <- lsmeans(m1g, "ReappraisalDirection", by = "fairLR",
               at = list(fairLR = c(-2.66,-1.22,0.22))) 
lsmDf <- lsm[1:9]
lsmDf <- as.data.frame(lsmDf)
lsmDf$fairLRrecode <- lsmDf$fairLR
lsmDf$fairLRrecode[lsmDf$fairLRrecode == -2.66] <- "Low (-2SD) Fairness"
lsmDf$fairLRrecode[lsmDf$fairLRrecode == -1.22] <- "Intermediate  Fairness"
lsmDf$fairLRrecode[lsmDf$fairLRrecode == 0.22] <- "High (+2SD) Fairness"
levels(lsmDf$fairLRrecode) <- c("High (+2SD) Fairness", "Intermediate  Fairness", "Low (-2SD) Fairness")
...
ggplot(lsmDf, aes(ReappraisalDirection, lsmean)) +
  ...
  facet_grid(~ fairLRrecode) +
  scale_x_discrete(labels = c("Baseline", "Empathy", "Punish"), expand=c(0,0.3)) +
  theme(axis.title.x = element_text(margin = margin(t = 15)),
        axis.title.y = element_text(margin = margin(r = 15)))

dev.off()

mean(allData$fairLR) + 2*sd(allData$fairLR)
...
write.csv(round(summary(m1g)$coefficients, 3), "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/NoIndividual/group4Interaction.csv")
...
mIRI2 <-
  ...
  AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + 
    ...
    scale(fairLR) * group4 + 
    scale(totalStake) * group4 +
    group4*scale(IRI_EMPATHETIC_CONCERN)*ReappraisalDirection + (1 | ID/block),
  family = binomial,
  data = allData,
  nAGQ = 0,  
  control = glmerControl(optimizer = "bobyqa"))
write.csv(round(summary(mIRI2)$coefficients, 3), "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/individual covariate models/IRIempathy_concern.csv")
...
mIRI3 <-
  ...
  AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + 
    ...
    scale(fairLR) * group4 + 
    scale(totalStake) * group4 +
    group4*scale(IRI_PERSONAL_DISTRESS)*ReappraisalDirection + (1 | ID/block),
  family = binomial,
  data = allData,
  nAGQ = 0,  
  control = glmerControl(optimizer = "bobyqa"))
# IRI affective empathy
write.csv(round(summary(mIRI3)$coefficients, 3), "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/individual covariate models/IRIdistress.csv")
...
mIRI_BIS <-
  ...
  AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + 
    ...
    scale(fairLR) * group4 + 
    scale(totalStake) * group4 +
    group4*scale(IRI_affEmpathy)*ReappraisalDirection +
    group4*scale(BIS_TOTAL)*ReappraisalDirection +
    (1 | ID/block),
  family = binomial,
  data = allData,
  nAGQ = 0,  
...
write.csv(round(summary(mIRI_BIS)$coefficients, 3), 
          "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/individual covariate models/IRIandBISTotal.csv")
...
mIRI_BISPLAN <-
  ...
  AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + 
    ...
    scale(totalStake) * group4 +
    group4*scale(IRI_affEmpathy)*ReappraisalDirection +
    group4*scale(BIS_NONPLAN)*ReappraisalDirection +
    (1 | ID/block),
  family = binomial,
  data = allData,
  nAGQ = 0,  
...
write.csv(round(summary(mIRI_BISPLAN)$coefficients, 3), 
          "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/individual covariate models/IRIandBISNonplan.csv")
summary(mIRI_BISPLAN)
...
tiff('/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/individual covariate models/SES*group4*UGcontext.tiff', 
     units="in", height = 8, width = 10, res=300)
lsm <- lsmeans(mIncome, "ReappraisalDirection", by = c("IncomePerCapita", "group4"), at = list(IncomePerCapita=c(10625,20500,31250)))
plot(lsm, horiz = F) +
  facet_grid(.~IncomePerCapita + group4)  +
  theme(axis.text.x=element_text(angle=90, hjust=1)) 
...
m1gEd_2 <-
  ...
  data = allData,
  control = glmerControl(optimizer = "bobyqa",optCtrl = list(maxfun = 2e4)))
lsm <- lsmeans(m1gm_2, "ReappraisalDirection", by = c("MMSE", "group4"), at = list(MMSE = c(26,15,30)))
ggplot(lsm, horiz = F) +
    theme(axis.text.x=element_text(angle=90, hjust=1)) +
    facet_grid(.~ MMSE + group4)
...
m1gEd_3 <-
  ...
  allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + 
    ...
    scale(fairLR) * group4 + 
    scale(totalStake) * group4 + 
    ReappraisalDirection*scale(EDUCATION)*group4 + 
    (1 | ID/block),
  family = binomial,
  data = allData,
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e4)))
lsm <- lsmeans(m1gm_2, "ReappraisalDirection", by = c("EDUCATION", "group4"), at = list(EDUCATION = c(26,28,30)))
ggplot(lsm, horiz = F) +
    theme(axis.text.x=element_text(angle=90, hjust=1)) +
    facet_grid(.~ MMSE + group4)
...
write.csv(round(summary(m1gEd_3)$coefficients,3), "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/individual covariate models/Education.csv")
```
...
noControl <- allData %>% filter(!group4 %in% c("Control"))
noControl$group4 <- factor(noControl$group4)
...
write.csv(round(summary(mBIS_total)$coefficients,3), '/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/individual covariate models/BIStotal2.csv')
...
tiff("/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/punishType/NoIndividual/UGcontext*group4.tiff", units="in", height = 5, width = 13, res=300)
...
lsmDf$group4 <- recode(lsmDf$group4, "attempter" = "Attempter", 
                             "control" = "Control",
...
ggplot(lsmDf, aes(ReappraisalDirectionResourcesRep, lsmean)) +
  ...
  theme(panel.spacing = unit(2, "lines")) +
  facet_grid(~ group4) +
  scale_x_discrete(labels = c("Baseline", "Empathy", "Reputation", "Resources"), expand=c(0,0.3)) +
  theme(axis.title.x = element_text(margin = margin(t = 15)),
        axis.title.y = element_text(margin = margin(r = 15)),
        panel.spacing = unit(2, "lines"),
        strip.text.x = element_text(size = 16))
...
marginal <- lsmeans(m1gPun, pairwise ~ ReappraisalDirectionResourcesRep: group4, adjust = "tukey")
m1gPunPairwise <- marginal$contrasts
...
The main model is 
$$
Accept\;Offer = Time \;lag + 
    Reappraisal\;DirectionResourcesRep * scale(fairLR) + \\
    ReappraisalDirectionResourcesRep * scale(totalStake) + 
    ReappraisalDirectionResourcesRep * group4 +\\
    scale(fairLR) * group4 +
    scale(totalStake) * group4 + (1 | ID/block)
...
emoRating <- within(emoRating, group4 <- relevel(group4, ref = "Control"))
...
tiff('/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/reactivity/NoIndividual/QfairnessContext1.tiff',
      units="in", height = 5, width = 12, res=300)
...
stargazer(summary(mFair)$coefficients, type = "text", 
          ...
          out = '/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/reactivity/NoIndividual/Qfairness_refControl.txt')
...
stargazer(summary(msympathy)$coefficients, type = "text", title = "Sympathy: scale(Rating) = PunishingType*group4 + (1|ID)",out = '/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/reactivity/NoIndividual/Qsympathy_refControl.txt')
...
ggplot(lsmDf, aes(PunishingType, lsmean)) +
  ...
  theme(axis.title.x = element_text(margin = margin(t = 15)),
        axis.title.y = element_text(margin = margin(r = 15)))
dev.off()
stargazer(summary(manger)$coefficients, type = "text",
          out = "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/reactivity/NoIndividual/Qanger_refControl.txt")
...
marginal <- lsmeans(manger, pairwise ~ ReappraisalDirection: group4, adjust = "tukey")
mangerPairwise <- marginal$contrasts
cld(marginal, alpha = 0.05, letters = letters, adjust = "tukey")
Anova(m1g)
summary(manger)
```
