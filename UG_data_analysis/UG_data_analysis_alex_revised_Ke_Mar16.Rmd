...
myVar <- c("EDUCATION", "GENDERTEXT", "PER.CAPITA.INCOME", "PROTECT2AGE", 
           ...
           "IRI_PERSPECTIVE_TAKING", "IRI_FANTASY_SCALE", "IRI_EMPATHETIC_CONCERN",
            "IRI_PERSONAL_DISTRESS", "IRI_cogEmpathy", "IRI_affEmpathy")
...
t1 <- createTable(c2, hide = c(Gender = "FEMALE", Race = "AFRICAN AMERICAN", Race = "ASIAN"), hide.no = 0, digits = 1, show.n = TRUE, show.p.mul = T)
t2_1 <- createTable(c2_2,hide = NA, hide.no = 0, digits = 1, show.n = TRUE, show.p.mul = T)

export2csv(t1, file = "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/group4IRI.csv")
...
```{r gamble acceptance}
fairAcceptPercDftmp <- allData[, c("ID", "AcceptOffer", "Fairness_score", "fairness", "group4")] %>%
  group_by(ID) %>%
  summarise(fairAcceptPerc = round(100*sum(AcceptOffer == 1 & fairness == "fair")/
              sum(fairness == "fair"), 2), 
            mostUnfairReject = round(100*sum(Fairness_score == 5 & AcceptOffer == 0)/
                                       sum(Fairness_score == 5), 2))
...
mean(fairAcceptPercDf$mostUnfairReject)
```
...
m1gBIS_1 <-
  ...
  allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + ReappraisalDirection *
  group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + 
    ReappraisalDirection*scale(BIS_NONPLAN)*group4 + (1 | ID/block),
  family = binomial,
  data = allData,
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e4)))
...
m1gBIS_2 <-
  ...
  allData$AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + ReappraisalDirection *
  group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + 
    scale(BIS_NONPLAN)*group4 + (1 | ID/block),
  family = binomial,
  data = allData,
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e4)))
stargazer(m1gBIS_1, type = "text", title = "AcceptOffer ~ ReappraisalDirection * scale(fairLR) * scale(totalStake) + ReappraisalDirection * group4 + scale(fairLR) * group4 + scale(totalStake) * group4 + ReappraisalDirection*scale(BIS_NONPLAN)*group4 + (1 | ID/block)", out = "/Volumes/LAB/Suicide_UG/UG_data_analysis/outputs/main/UGcontent*BIS_NONPLAN*group4.text")
